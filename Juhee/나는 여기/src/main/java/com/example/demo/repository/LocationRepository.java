...
public interface LocationRepository extends JpaRepository<Location,Integer> {
    ...
    public List<Location> getLocationsByUserIdAndCreated_atEquals(Integer uid, Timestamp date);

    @Query(value = "select * FROM location where date_format(created_at, '%Y-%m-%d')=date_format(?1, '%Y-%m-%d') and user_id!=?2",nativeQuery = true)
    public List<Location> getLocationsByCreated_atEqualsAndUserIdIsNot(Timestamp date,int uid);
    ...
    @Query(value = "select * from location " +
            ...
            "order by user_id"
            ,nativeQuery = true)
    public List<Location> getLocationsByUserIdAndCreated_atBetween(int uid,Date date);
